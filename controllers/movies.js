  ...
  Movie.create({
    ...
    .catch((err) => {
      if (err.name === 'ValidationError') {
        next(new ValidationError('Внесены некорректные данные при создании карточки'));
      } else {
        next(err);
      }
    });
  ...
  Movie.findById(req.params.id)
    ...
    .catch((err) => {
      if (err.name === 'CastError') {
        next(new ValidationError('Отправлены некорректные данные для удаления карточки'));
      } else {
        next(err);
      }
    });
